...
if ( ! class_exists( "Kundenarchiv_Filter" ) ) { // class that handles the filter functionality
	class Kundenarchiv_Filter {
		...
		public function __construct() {
			...
			add_action( 'posts_join', array(
				$this,
				'join_tables_to_add_custom_sql'
			), 9999 ); // join meta tables to perform custom sql query

			add_filter( 'posts_where', array( $this, 'append_custom_sql' ), 10, 2 );  // add custom sql query
		}

		public function join_tables_to_add_custom_sql( $join ) {
			global $wp_query, $wpdb;
			if ( $wp_query->query_vars['post_type'] == "kundenarchiv" ) {
		...
		public function append_custom_sql( $where = '', $wp_query_obj ) {
			if ( $wp_query_obj->query_vars['post_type'] == "kundenarchiv" ) {
				if ( ! empty( $this->from ) && ! empty( $this->until ) ) {
					$where .= "AND meta_1.meta_key = 'von' AND meta_2.meta_key = 'bis' AND ( ((meta_1.meta_value BETWEEN $this->from AND $this->until) AND (meta_2.meta_value BETWEEN $this->from AND $this->until))
					OR ((meta_1.meta_value BETWEEN $this->from AND $this->until) AND meta_2.meta_value<1) OR ((meta_2.meta_value BETWEEN $this->from AND $this->until) AND meta_1.meta_value<1)
					)";
				} else {
					if ( ! empty( $this->from ) ) {
						$where .= "AND meta_1.meta_key = 'von' AND meta_2.meta_key = 'bis' 
						AND ( 
							(meta_1.meta_value >= $this->from AND meta_2.meta_value >= $this->from)
							OR 
							(meta_1.meta_value >= $this->from AND meta_2.meta_value<1) 
							OR 
							(meta_2.meta_value >= $this->from AND meta_1.meta_value<1)
					) AND (meta_1.meta_value>0 OR meta_2.meta_value>0)";
					}
					if ( ! empty( $this->until ) ) {
						$where .= "AND meta_1.meta_key = 'von' AND meta_2.meta_key = 'bis' 
						AND ( 
							(meta_1.meta_value <= $this->until AND meta_2.meta_value <= $this->until)
							OR 
							(meta_1.meta_value <= $this->until AND meta_2.meta_value<1) 
							OR 
							(meta_2.meta_value <= $this->until AND meta_1.meta_value<1)
					) AND (meta_1.meta_value>0 OR meta_2.meta_value>0)";
					}
				}
			}
		...
		public function custom_query_vars( $query ) {
			...
			if ( ! empty( $this->suche ) || ! empty( $this->suchen_in ) ) { // set query accordingly if from or until year is set in filter
				$meta_query             = array();
